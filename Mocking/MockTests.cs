...
namespace Mocking
{
    ...
    public class MockTests
    {
        ...
        public void CanCalulateAverageWeeklySaleryPerEmployee()
        {
            ...
            var user     = new User() { UserName = "sist@eal.dk", Password = "!QAZ2wsx", NumMessagesCreated = 0 };
            var mock     = new Mock<IEmployeeRepository>();
            ...
            int    hours               = 42;
            int    quantity            = 3;
            double expected            = ((1000 * 42) + (750 * 42) + (500 * 42)) / quantity;
            double averageWeeklySalary = 0;

            List<Employee> employees = mock.Object.FindAllEmployees();

            foreach(Employee e in employees)
            {
                averageWeeklySalary += e.CalculateWeeklyAverageSalary(hours, quantity);
            }

            Assert.AreEqual(expected, averageWeeklySalary);
        }
       ...
       // [TestMethod]
        ...
        [TestMethod]
        public void CanGetMailById()
        {
            const int id = 1;

            var mock = new Mock<IEmployeeRepository>();
            mock.Setup(m => m.LoadEmployee(id))
                .Returns(() => new Employee() { Id = id, Name = "Pelle", Type = "Student", Wage = 1, MailBox = new MailBox() { NumReceivedMessages = 0 } });

            Employee e = mock.Object.LoadEmployee(id);
            var message = new Mail() { Content = "Hello " + e.Name + " Your paycheck is wrong!" };

            var mockMail = new Mock<IMailModule>();
            mockMail.Setup(x => x.GetEmailById(id))
               .Returns(() => new Mail()
               { Content = "Hello " + e.Name + " Your paycheck is wrong!" });

            //  Act
            Mail mailById = mockMail.Object.GetEmailById(id);

            //  Assert
            Assert.AreEqual("Hello " + e.Name + " Your paycheck is wrong!", mailById.Content);
        }
        /*
